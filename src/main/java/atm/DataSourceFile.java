...
public class DataSourceFile implements DataSource {
     ...
     */
    //ไม่ต้อง Autowireเพราะไม่มีconponant
    public DataSourceFile(@Value("${filename}") String filename) {
        this.filename = filename;
    }
    //@Value("${filename}" คือให้ไปอ่่าน filename จากconfig file(value.properties)ซึ่งในนั้น filename จะโยงไปถึงไฟล์customer.txt
    //ตัวแปรที่เป็นprimative typeต้องเติม@value
    /**
